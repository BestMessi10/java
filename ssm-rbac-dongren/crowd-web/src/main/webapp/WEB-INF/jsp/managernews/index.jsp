...
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/managernews/add'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
   ...
   <li><a href="#ios" data-toggle="tab">用户发布信息列表（待审核）</a></li>
        ...
        <script type="text/javascript">
         ...
         // 分页查询-查询用户发布的
            function pageQuery1( pageno ) {
            	...
            	$.ajax({
            		type : "POST",
            		url  : "${APP_PATH}/managernews/pageQuery1",
            		data : jsonData,
            		beforeSend : function(){
            		...
            		success : function(result) {
            			...
            			if ( result.success ) {
            				...
            				if ( pageno > 1 ) {
            					pageContent += '<li><a href="#" onclick="pageQuery1('+(pageno-1)+')">上一页</a></li>';
            				}
            				...
            				for ( var i = 1; i <= dongNewsPage.totalno; i++ ) {
            					if ( i == pageno ) {
            						...
            						pageContent += '<li ><a href="#" onclick="pageQuery1('+i+')">'+i+'</a></li>';
            					}
            				}
            				
            				if ( pageno < dongNewsPage.totalno ) {
            					pageContent += '<li><a href="#" onclick="pageQuery1('+(pageno+1)+')">下一页</a></li>';
            				}

            				$("#dongNewsData1").html(tableContent);
            ...
            function pageQuery( pageno ) {
            	...
            	$.ajax({
            		type : "POST",
            		url  : "${APP_PATH}/managernews/pageQuery",
            		data : jsonData,
            		beforeSend : function(){
            			loadingIndex = layer.msg('处理中', {icon: 16});
            		},
            ...
            function goUpdatePage(id) {
            	window.location.href = "${APP_PATH}/managernews/edit?id="+id;
            }
            function goNotAssignPage( id, title) {
				...
				layer.confirm("确认取消审核新闻【"+title+"】通过, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
    				...
    				$.ajax({
    					type : "POST",
    					url  : "${APP_PATH}/managernews/assign",
    					data : { id : id ,
    							flag: flag	
    					},
    					success : function(result) {
            ...
            function deleteDongNewss() {
            	...
            	if ( boxes.length == 0 ) {
        			...
        			layer.confirm("删除选择的用户信息, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
        				...
        				$.ajax({
        					type : "POST",
        					url  : "${APP_PATH}/managernews/deletes",
            ...
            function deleteDongNews( id, title ) {
    			layer.confirm("删除新闻信息【"+title+"】, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
    				...
    				$.ajax({
    					type : "POST",
    					url  : "${APP_PATH}/managernews/delete",
    					data : { id : id },
    					success : function(result) {
    						if ( result.success ) {
    							pageQuery(1);
