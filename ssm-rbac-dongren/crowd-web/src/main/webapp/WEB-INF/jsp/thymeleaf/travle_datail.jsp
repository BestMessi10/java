<%@page pageEncoding="UTF-8"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title></title>
		<link rel="stylesheet" type="text/css" href="../css/style.css"/>
	</head>
	<body class="drly_detail detail"  style='background: url(../img1/bg_con.jpg) no-repeat;background-size: cover;'>
		<header>
			<div class="layout">
				<a href="index.html" class="logo">
					<img src="../img/logo.png"/>
					<span class="italic">侗人网</span>
				</a>
				<ul class="nav">
					<li><a href="index">首页</a></li>
					<c:forEach items="${dongColumns }" var="dongColumn">
						<li><a href="${dongColumn.url }">${dongColumn.name}</a></li>
					</c:forEach>
				</ul>
			</div>
		</header>
		<div class="layout">
			<a href="" class="bold">侗人旅游</a><!--<span>&gt;</span>--><i class="iconfont icon-jiantouyou"></i><a href="/">正文</a>
			<div class="detail_title">
				<p class="text-center size34 bold">习近平主席赴俄罗斯出席第四届东方经济论坛</p>
				<span class="color666 newsname">新浪新闻</span>
				<span class="color666">2018-05-11&nbsp;&nbsp;15:34:22</span>
			</div>
			<div id="focusImage" class="slide">
		  		<ul class="contents">
		            <li>
			           	<div class="image">
				           	<img src="../img/td_img1.jpg">
			           	</div>
			           	<div class="text">	                
			           		<p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
			           	</div>
		      		</li>
		      		<li>
			           	<div class="image">
				           	<img src="../img/c_img10.jpg">
			           	</div>
			           	<div class="text">	                
			           		<p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
			           	</div>
		      		</li>
		      		<li>
			           	<div class="image">
				           	<img src="../img/td_img1.jpg">
			           	</div>
			           	<div class="text">	                
			           		<p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
			           	</div>
		      		</li>
		      		<li>
			           	<div class="image">
				           	<img src="../img/c_img10.jpg">
			           	</div>
			           	<div class="text">	                
			           		<p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
			           	</div>
		      		</li>
		      		<li>
			           	<div class="image">
				           	<img src="../img/td_img1.jpg">
			           	</div>
			           	<div class="text">
			                <p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
		
			           	</div>
		      		</li>
		      		<li>
			           	<div class="image">
				           	<img src="../img/c_img10.jpg">
			           	</div>
			           	<div class="text">
			                
			                <p>青岛何时划为国家中心城市、流亭机场停用后未来规划、欢乐滨海城发展前景……这些你关心的热点问题，官方都回复了！26日下午，青岛市规划局党组成员副局长滕军红做客青岛新闻网民生在线围绕“聚焦民生，提升效能主题与网民进行在线交流。此次访谈中滕军红表示省委省政府已明确支持青岛建设国家中心城市，并重点介绍了包括红岛经济区未来之城、老沧口、楼山后等九大片区未来的规划内容。</p>
			           	</div>
		      		</li>
		       
		        </ul>
		        <div class="swiper_num">
		        	<span class="num">1</span>
		        	<span class="line"></span>
		        	<span class="all_num"></span>
		        </div>
				<span class="link-watch pre down" ></span>
				<span class="link-watch next down" ></span>
			</div>
		</div>
		<footer>
			<div class="fd-con">
	            <div class="layout">
	                <a href="index.html" class="fd-logo">
	                	<img src="../img/logo.png">
	                	<span class="bold size18 italic color666">侗人网</span>
	                </a>
	                <div class="fd-nv">
	                    <a href="index.html">首页</a>
	                    <a href="flash.html">侗人快讯</a>
	                    <a href="culture.html">侗人文化</a>
	                    <a href="economics.html">侗人经济</a>
	                    <a href="travel.html" >侗人旅游</a>
	                    <a href="video.html">侗人视频</a>    
	                </div>
	                <div class="qr">
	                	<img src="http://www.i-vista.org/filespath/images/20170814170010.jpg" data-original="/filespath/images/20170814170010.jpg" style="transition: all 1s ease 0s; opacity: 1;">
	                </div>
	            </div>
            </div>
            <div class="fd-copy">
	           	<div class="layout">&copy;万宝源</div>
        	</div>
        </div>
		</footer>
		<script src="../js/jquery-1.11.0.js" type="text/javascript" charset="utf-8"></script>
		<script src="../js/qwrap.js" type="text/javascript" charset="utf-8"></script>
		<script src="../js/lunbo.js" type="text/javascript" charset="utf-8"></script>
		<script type="text/javascript">
			window.onload = function(){
				$(".contents li").eq(0).addClass("current")
				$(".all_num").text($(".contents li").length)
				
			}
			new ImageSlide({
				project:"#focusImage",
				content:".contents li",
				watch:".link-watch",
//				auto:!0,
				hide:!0
			})
		</script>
	</body>
</html>
